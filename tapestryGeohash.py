...
geohash_polygons.gpd = gpd.GeoDataFrame(geometry=geo, data=geohash_polygons, crs={'init': 'epsg:4326'})
...
###### Intersection points w. geohash_polygons
idx = index.Index()
for index, row in geohash_polygons.gpd.iterrows():
    idx.insert(index,row.geometry.bounds)

def seba_over(point):
    poly_idx = [i for i in idx.intersection((point.coords[0]))
                if point.within(geohash_polygons.gpd.geometry[i])]
    if poly_idx:
...
points['geohashID'] = poly_ids

x = points[['type','geohashID']]
y = geohash_polygons.gpd[['geohash_label']]
y['geohashID'] = geohash_polygons.gpd.index

data = x.merge(y, on='geohashID')
data['values'] = 1

pt = data.groupby(['geohash_label', 'type'])['values'].sum().unstack()
...
labels['geohash_label'] = labels.index
geohash_polygons.gpd = geohash_polygons.gpd.merge(labels, on='geohash_label', how='left')
geohash_polygons.gpd.to_csv('Tapestry_{}.txt'.format(msatag), sep='\t', index=False)
